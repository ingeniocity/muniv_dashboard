...
exports.insertinstitutedata=function(db,res,features,callbackdata) //instituteConfig is a json array containing Institute name and code
{
	...
	},function(e,docs)
	{
		if(docs) 
		{
			...
			},function(e1,docs1)
			{
				if(docs1)
				{
					res.send({sucess:true,data:"Sucessfully Updated"})	
				}
				else
				{
...
exports.returnInstituteFeatures=function(db,res,info)
{
	...
	instFeature.findOne({"instituteInfo.instCode":instCode},{}, function(e, docs) 
	{
		if(docs)
		{
			res.send({sucess:true,data:docs});
		}
		else
		{
			res.send({success: false, data: ""});
