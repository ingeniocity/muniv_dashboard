	...
	var key='',salt = '', superSet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.@$%';
		...
		"oAuthenticationSetup":features[0].otherFeatures.authenticationSetup
			...
			if(docs)
			{
				...
				if(docs1)
				{
					var data=[{"key":docs1.key,"salt":docs1.salt}];
...
exports.updatefeatures=function(db,res,req,features,callbackfun) 
{
...
exports.insertotp=function(db,res,req,info,callbackinsertotp)
{
	var otpConfig=db.get('otpInfo');
	console.log("insise insert otp config");
	var today =new Date();
	...
	},function(e,docs)
	{
		if(docs)
		{
			...
			errorcode="";
			console.log("after insert");
			callbackinsertotp(res,req,status,errorcode);
		}
		else
		{
			console.log("unable to insert");
			status="faliure";
			errorcode="UNABLETOSENDMAIL";
			callbackinsertotp(res,req,status,errorcode);
		}
	});
	
};
exports.otpExists=function(db,res,req,info, callbackotp)
{
	var otpConfig=db.get('otpInfo'); 
	console.log(info);
	console.log("inside otp exists function");
	var empNumber=info[0].employeeNumber;
	console.log("employee number is "+empNumber);
	otpConfig.findOne({"employeeNumber":empNumber},{}, function(e, docs) 
	{
		if(docs){
			console.log(docs);
			var today=new Date();
			var milli=today.getTime();
			console.log("Date is"+milli);
			var diff=milli-docs.otpTime;
			console.log(diff);
			if(diff<86400000)
			//if(docs.otpEnable == true)
			{
				var data=docs.otp;
				console.log("otp already exists");
				var message="OTP Already Generated";
				otpConfig.remove({employeeNumber:empNumber},function(e,docs){
				
				});
				callbackotp(res, req, data,message);
			}
			else
			{
				var data="";
...
exports.checkotp=function(db,res,info )
{
	console.log("inside check One Time Otp");
	var otpConfig=db.get('otpInfo'); // otpInfo is our collection
	console.log(info);
	var empNumber=info[0].employeeNumber;
	console.log("Employee number is "+empNumber);
	var otp = info[0].otp;
	...
	otpConfig.findOne({"employeeNumber":empNumber},{}, function(e, docs) 
	{
		if(docs)
		{
			...
			//if(docs.otp === otp && docs.otpEnable==true)
			if(docs.otp=== otp && diff<86400000)
			{
				res.send({status:"success",data:"",errorcode:""});
				otpConfig.remove({employeeNumber:empNumber},function(e,docs){
				
				});
			}
		
			else if(docs.otp===otp)
			{
				res.send({status:"failure",data:"OTP Expired",errorcode:"OTPEXPIRED"});	
			}
			else
			{
				res.send({status:"failure",data:"OTP is Invalid",errorcode:"OTPINVALID"});
			}	
		}
		else
		{
			res.send({success:"failure",data:"OTP not generated",errorcode:"OTPNOTGENERATED"});
		}
			
	});
};		

exports.checkks=function(db,res,req,info,callbackcheckks)
{
	var instCode=info[0].c;
	var key=info[0].k;
	var salt=info[0].s;
	console.log("inside checkks");
	console.log(instCode);
	console.log(key);
	console.log(salt);
	var instConfig=db.get('Institute_Configuration');
	instConfig.findOne({"instituteCode":instCode},{},function(e,docs){
		if(docs)
		{
			console.log("docs :"+docs);
			if(docs.key===key && docs.salt===salt)
			{
				console.log("matched");
				status="success";
				errorcode="";
				data="";
				callbackcheckks(res,req,status,data,errorcode);
			}
			else
			{
				console.log("not");
				status="failure";
				errorcode="KEYSALTERROR";
				data="key salt error";
				callbackcheckks(res,req,status,data,errorcode);
			}
		}
...
exports.getdatainstitute=function(db, res, instC)
{
	...
	var data=instFeature.find({instCode:instC},{}, function(e, docs) 
	{
		if(docs)
		{
			console.log("docs is here");
			console.log(docs);
			console.log(docs[0].gGeneralUniversityInfo);	
			res.send({success: true, data: docs});
			return;
		}
...
exports.getdatainstituteks=function(db, res, qstr)
{
	...
	instConfig.find({"instituteCode" : qstr.i},{}, function(e, docs){
		...
		console.log(docs[0].key);
		    ...
		    console.log("Matched");
	            instFeature.find({"instCode" : qstr.i}, {"_id" :0}, function (e1, docs1){
	                console.log(docs1);
