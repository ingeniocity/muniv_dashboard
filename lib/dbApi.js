...
exports.checkotp=function(db,res,info )
{
	...
	otpConfig.findOne({"employeeNumber":empNumber},{}, function(e, docs) 
	{
		if(docs)
		{
			...
			{
				res.send({status:"faliure",data:"OTP Expired",errorcode:"OTPEXPIRED"});	
			}
			else
			{
				res.send({status:"faliure",data:"OTP is Invalid",errorcode:"OTPINVALID"});
			}	
		}
		else
		{
			res.send({success:"faliure",data:"OTP not generated",errorcode:"OTPNOTGENERATED"});
		}
